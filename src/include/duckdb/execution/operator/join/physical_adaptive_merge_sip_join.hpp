...
class PhysicalAdaptiveMergeSIPJoin : public PhysicalComparisonJoin {
public:
	PhysicalAdaptiveMergeSIPJoin(ClientContext &context, LogicalOperator &op, unique_ptr<PhysicalOperator> left,
	                             unique_ptr<PhysicalOperator> right, vector<JoinCondition> cond, JoinType join_type,
	                             vector<idx_t> &left_projection_map, vector<idx_t> &right_projection_map,
	                             vector<idx_t> &merge_projection_map, bool enable_nlaj);
...
private:
	...
	void GenerateZoneFilter(Vector &key_vector, idx_t count);
	void PassZoneFilter();
	...
	idx_t DoLookups(ClientContext &context, DataChunk &right_chunk, DataChunk &right_condition_chunk,
