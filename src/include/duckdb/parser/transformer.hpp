...
class Transformer {
public:
	explicit Transformer(Transformer *parent = nullptr) : parent(parent), anonymous_variable_idx(0) {
		query_graph = make_unique<TransformerQueryGraph>();
	}
...
private:
	...
	//! Anonymous path variable index used to generate anonymous variable names.
	idx_t anonymous_variable_idx;
	//! Query graph constructed from path patterns.
	unique_ptr<TransformerQueryGraph> query_graph;
...
private:
	//! Helper functions
	VertexRef *ConstructVertex(duckdb_libpgquery::PGNodePattern *node);
...
private:
	...
	//! Transform a Postgres T_PGCreateVertexStmt node into a CreateStatement
	unique_ptr<CreateStatement> TransformCreateVertex(duckdb_libpgquery::PGNode *node);
	//! Transform a Postgres T_PGCreateEdgeStmt node into a CreateStatement
	unique_ptr<CreateStatement> TransformCreateEdge(duckdb_libpgquery::PGNode *node);
	//! Transform a Postgres T_PGCreateStmt node into a CreateStatement
	...
	unique_ptr<ParsedExpression> TransformValue(duckdb_libpgquery::PGValue val);
	...
	//! Transform a path pattern into a join node
	unique_ptr<TableRef> TransformPathPattern(duckdb_libpgquery::PGPathPattern *path_pattern);
