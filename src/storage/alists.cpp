...
// void AList::AppendPKFK(Vector &source, Vector &edges, idx_t count, EdgeDirection direction) {
...
// void AList::Append(Vector &source, Vector &edges, Vector &target, idx_t count, EdgeDirection direction) {
...
//		forward_map[source_rid]->size++;
//	}
//	if (direction == EdgeDirection::SELF || direction == EdgeDirection::UNDIRECTED) {
//		for (idx_t i = 0; i < count; i++) {
//			auto source_idx = source_data.sel->get_index(i);
//			auto target_idx = target_data.sel->get_index(i);
//			if ((*source_data.nullmask)[source_idx] || (*target_data.nullmask)[target_idx]) {
...
// void AList::Finalize(EdgeDirection direction) {
//	if (direction == EdgeDirection::PKFK) {
...
// static idx_t BuildZoneFilterInternal(CompactList &alist, data_ptr_t *hashmap, idx_t size, bitmask_vector
// &zone_filter) { 	idx_t matched_count = 0; 	for (idx_t i = 0; i < size; i++) { 		if (*((data_ptr_t *)(hashmap +
// i))) { auto
// offset = alist.offsets[i]; 			auto length = alist.sizes[i]; 			matched_count += length; 			for (idx_t idx =
// 0;
// idx < length; idx++) { 				auto edge_id = alist.edges[idx + offset]; 				auto zone_id = edge_id /
// STANDARD_VECTOR_SIZE; 				auto index_in_zone =
// edge_id - (zone_id * STANDARD_VECTOR_SIZE); 				if (!zone_filter[zone_id]) {
...
// { 	idx_t matched_count = 0; 	for (idx_t i = 0; i < size; i++) { 		if (*((data_ptr_t *)(hashmap + i))) {
// auto offset = alist.offsets[i]; 			auto length = alist.sizes[i]; 			for (idx_t idx = 0; idx < length;
// idx++) {
