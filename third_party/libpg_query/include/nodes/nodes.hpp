...
typedef enum PGNodeTag {
	...
	// TAGS FOR PATH PATTERN IN PARSE TREE NODES (parsenodes.h)
	T_PGPathPattern,
	T_PGNodePattern,
	T_PGPatternElement,
	T_PGRelRange,
...
typedef enum PGJoinType {
	 ...
	 */
	PG_JOIN_PATH, /* join for PATH PATTERN */
} PGJoinType;
...
#define IS_OUTER_JOIN(jointype)                                                                                        \
	(((1 << (jointype)) & ((1 << PG_JOIN_LEFT) | (1 << PG_JOIN_FULL) | (1 << PG_JOIN_RIGHT) | (1 << PG_JOIN_ANTI))) != \
	 0)
