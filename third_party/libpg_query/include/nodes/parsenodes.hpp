 ...
 * Graph path pattern
 * */
typedef struct PGNodePattern {
	PGNodeTag type;
	char *alias;
	char *label;
} PGNodePattern;

enum PGRelDirection { FORWARD_DIRECTION, BACKWARD_DIRECTION };

// -1 means not specified.
typedef struct PGRelRange {
	PGNodeTag type;
	PGValue *length_low;
	PGValue *length_high;
} PGRelRange;

typedef struct PGPatternElement {
	PGNodeTag type;
	PGRelDirection direction;
	char *rel_alias;
	char *rel_label;
	PGNode *rel_range;
	PGNode *node_pattern;
...
#define GetCTETargetList(cte)                                                                                          \
	(AssertMacro(IsA((cte)->ctequery, PGQuery)), ((PGQuery *)(cte)->ctequery)->commandType == PG_CMD_SELECT            \
...
typedef struct PGCreateVertexStmt {
	PGNodeTag type;
	PGRangeVar *name;
	PGRangeVar *table;
} PGCreateVertexStmt;

typedef struct PGCreateEdgeStmt {
	PGNodeTag type;
	PGRangeVar *name;
	PGRangeVar *table;
	PGRangeVar *from_ref;
...
typedef struct PGIntervalConstant {
	PGNodeTag type;
	int val_type;    /* interval constant type, either duckdb_libpgquery::T_PGString, duckdb_libpgquery::T_PGInteger or
	                    duckdb_libpgquery::T_PGAExpr */
